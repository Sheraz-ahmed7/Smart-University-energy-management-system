...
public class ChartsPanel extends JPanel {
    ...
    private JPanel buildControls() {
        ...
        deptCombo.setPreferredSize(new Dimension(210, 36));
        row.add(deptCombo);
        ...
        Theme.styleCombo(rangeCombo);
        rangeCombo.setPreferredSize(new Dimension(150, 36));
        row.add(rangeCombo);
    ...
    private JPanel wrapChart(JFreeChart chart) {
        ...
        cp.setBackground(CHART_BG);
        cp.setPopupMenu(null);
        cp.setMouseWheelEnabled(true);
        JPanel p = new JPanel(new BorderLayout());
        p.setBackground(CHART_BG);
    ...
    private void styleCategoryPlot(CategoryPlot plot) {
        ...
        plot.getDomainAxis().setLabelPaint(AXIS_LABEL);
        plot.getDomainAxis().setTickLabelPaint(AXIS_TICK);
        plot.getDomainAxis().setLabelFont(Theme.FONT_SMALL);
        plot.getDomainAxis().setTickLabelFont(Theme.FONT_SMALL);
        plot.getRangeAxis().setLabelPaint(AXIS_LABEL);
        plot.getRangeAxis().setTickLabelPaint(AXIS_TICK);
        plot.getRangeAxis().setLabelFont(Theme.FONT_SMALL);
        plot.getRangeAxis().setTickLabelFont(Theme.FONT_SMALL);
    }
    ...
    private JFreeChart styledChart(JFreeChart chart) {
        ...
        chart.getTitle().setFont(Theme.FONT_HEADING);
        if (chart.getLegend() != null) {
            chart.getLegend().setBackgroundPaint(CHART_BG);
            chart.getLegend().setItemPaint(Theme.TEXT_SECONDARY);
            chart.getLegend().setItemFont(Theme.FONT_SMALL);
        }
        return chart;
    }

    // ── 1. Consumption bar chart ───────────────────────────────
    private JFreeChart buildConsumptionChart() {
        ...
        for (Department d : Queries.getAllDepartments()) {
            ...
            ds.addValue(kwh, "Monthly kWh", d.getDeptName());
        }
        JFreeChart chart = ChartFactory.createBarChart(
            "Monthly Energy Consumption by Department",
            "Department", "kWh", ds, PlotOrientation.VERTICAL, false, true, false);
        styledChart(chart);
        CategoryPlot plot = chart.getCategoryPlot();
        styleCategoryPlot(plot);
        ((BarRenderer) plot.getRenderer()).setSeriesPaint(0, SERIES_CLR[0]);
        ((BarRenderer) plot.getRenderer()).setSeriesOutlinePaint(0, SERIES_CLR[0].darker());
    ...
    private JFreeChart buildDeviceChart() {
        ...
        List<Department> depts = (sel != null && sel.getDeptId() != 0)
            ? List.of(sel) : Queries.getAllDepartments();
        ...
        plot.setBackgroundPaint(PLOT_BG);
        plot.setOutlinePaint(GRID_LINE);
        plot.setLabelBackgroundPaint(new Color(30, 46, 34));
        plot.setLabelOutlinePaint(GRID_LINE);
        plot.setLabelShadowPaint(null);
        plot.setLabelPaint(Theme.TEXT_PRIMARY);
        plot.setLabelFont(Theme.FONT_SMALL);
        Color[] pc = {SERIES_CLR[0],SERIES_CLR[1],SERIES_CLR[2],SERIES_CLR[3]};
        int i=0;
        for (var key : ds.getKeys()) plot.setSectionPaint(key, pc[i++%pc.length]);
        return chart;
    }
    ...
    private JFreeChart buildCostChart() {
        DefaultCategoryDataset ds = new DefaultCategoryDataset();
        for (Department d : Queries.getAllDepartments()) {
            double cost = EnergyCalculator.calculateMonthlyCost(
                Queries.getTotalMonthlyKWhByDepartment(d.getDeptId()));
            ds.addValue(cost, "Monthly Cost (Rs)", d.getDeptName());
        }
        JFreeChart chart = ChartFactory.createBarChart(
            "Monthly Electricity Cost by Department",
            "Department", "Cost (Rs)", ds, PlotOrientation.VERTICAL, false, true, false);
        styledChart(chart);
        CategoryPlot plot = chart.getCategoryPlot();
        styleCategoryPlot(plot);
    ...
    // ── 4. Trend chart — real DB data, fallback to dept-based estimate ─
    private JFreeChart buildTrendChart() {
        ...
        for (Department d : depts) {
            ...
            List<EnergyUsage> history = Queries.getEnergyUsageHistory(d.getDeptId(), days);
            ...
            if (!history.isEmpty()) {
                ...
                // Fallback: deterministic estimate based on current device load
                // No Random() — uses sinusoidal seasonal variation seeded by deptId
                double base = Queries.getTotalMonthlyKWhByDepartment(d.getDeptId()) / 30.0;
                for (int i = days; i >= 0; i--) {
                    LocalDate ld = LocalDate.now().minusDays(i);
                    Day day = new Day(ld.getDayOfMonth(), ld.getMonthValue(), ld.getYear());
                    // Deterministic variation: sine wave based on day-of-year + deptId seed
    ...
    // ── 5. Carbon footprint bar ────────────────────────────────
    private JFreeChart buildCarbonChart() {
        DefaultCategoryDataset ds = new DefaultCategoryDataset();
        for (Department d : Queries.getAllDepartments()) {
            double co2 = CarbonCalculator.calculateCarbonFootprint(
                Queries.getTotalMonthlyKWhByDepartment(d.getDeptId()));
            ds.addValue(co2, "CO₂ Emissions (kg)", d.getDeptName());
        }
        JFreeChart chart = ChartFactory.createBarChart(
            "Monthly Carbon Footprint by Department",
            "Department", "CO₂ (kg)", ds, PlotOrientation.VERTICAL, false, true, false);
        styledChart(chart);
        CategoryPlot plot = chart.getCategoryPlot();
        styleCategoryPlot(plot);
    ...
    // ── 6. Efficiency line chart — deterministic, no Random() ─
    private JFreeChart buildEfficiencyChart() {
        DefaultCategoryDataset ds = new DefaultCategoryDataset();
        for (Department d : Queries.getAllDepartments()) {
            double kwh = Queries.getTotalMonthlyKWhByDepartment(d.getDeptId());
            int score = efficiencyScore(kwh);  // deterministic — no Math.random()
            ds.addValue(score, "Efficiency Score", d.getDeptName());
        }
        JFreeChart chart = ChartFactory.createLineChart(
            "Department Efficiency Scores",
            "Department", "Score (0–100)", ds, PlotOrientation.VERTICAL, false, true, false);
        styledChart(chart);
        CategoryPlot plot = chart.getCategoryPlot();
        styleCategoryPlot(plot);
        plot.getRangeAxis().setRange(0, 100);
        LineAndShapeRenderer rend = new LineAndShapeRenderer(true, true);
        rend.setSeriesPaint(0, SERIES_CLR[1]);
